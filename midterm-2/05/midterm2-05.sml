...
fun perm_capture_231(xs: int list): bool = (
    let 
        fun helper1(a, xs): bool = (
            if list_forall(xs, fn(b) => b > a) then false 
            else true
        )

        fun helper2(b, xs): bool = (
              if list_forall(xs, fn(c) => c > b) then false 
              else true
        )
    in 
        ...
        | x1::xs => (
            case xs of nil => false 
            | x2 :: xs => if helper1(x1, x2::xs) andalso helper2(x1, xs) then true 
            else perm_capture_231(x2::xs)
        )
    end
)
